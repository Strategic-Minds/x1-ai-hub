...
export default function FactoryCard({ factory }) {
  return (
    <article className="xa-card-feature relative">
      {factory.locked && (
        <span className="absolute right-3 top-3 rounded-full border border-primary/40 px-2 py-0.5 text-[10px] font-bold uppercase tracking-wide text-primary opacity-60">Locked</span>
      )}
      <div className="xf-icon"><Icon name={factory.icon} /></div>
      <h3>{factory.name}</h3>
      <p>{factory.description}</p>
      <div className="mt-3 text-xs font-semibold uppercase tracking-wide text-muted-foreground">{factory.category}</div>
    </article>
  );
}