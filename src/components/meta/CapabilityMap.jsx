...
export default function CapabilityMap({ items }) {
  ...
  return (
    <div className="grid grid-cols-2 gap-2 sm:grid-cols-3 lg:grid-cols-4">
      {items.map((c, i) => (
        <div key={c.key || c.category || i} className="rounded-lg border border-border bg-white px-3 py-2">
          <div className="truncate text-[10px] font-bold uppercase tracking-wide text-muted-foreground">{c.name || c.category}</div>
          ...
          {c.executor && (
            <div className="mt-1 truncate text-[9px] text-muted-foreground">→ {c.executor}</div>
          )}
        </div>
      ))}
    </div>
